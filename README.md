...
- `signed-debug-apk` に job-level `if: ${{ secrets.* }}` を置くと GitHub Actions の workflow evaluation 段階で落ちるため、secret readiness の判定は caller 側では行わない方針です。
- この実行環境からは GitHub への直接アクセスが 403 で制限されていたため、reusable workflow ref の immutable SHA pin は次回ネットワーク制約のない環境で実施してください。
