...
- `.github/workflows/build-android-debug-apk.yml` は 2段構成です。
  ...
  - `signed-debug-apk`: 同一 workflow 内で secrets を実行時に検証し、debug keystore を復号して `assembleDebug` を再実行
- caller workflow では `permissions: contents: read` を明示しています。
- `signed-debug-apk` の secret readiness 判定は step 内の shell で実施します。`jobs.<job_id>.if` や external reusable workflow の解決段階で止めないことで、「0s failure / jobs 未開始」の評価エラーを避ける狙いです。
- CI で署名付き debug APK を作る場合は、以下の 4 secrets を設定してください。
  - `DEBUG_KEYSTORE_BASE64`
  - `DEBUG_KEYSTORE_PASSWORD`
  - `DEBUG_KEY_ALIAS`
  - `DEBUG_KEY_PASSWORD`
- `app/build.gradle.kts` は `CI_DEBUG_KEYSTORE_FILE` / `CI_DEBUG_KEYSTORE_PASSWORD` / `CI_DEBUG_KEY_ALIAS` / `CI_DEBUG_KEY_PASSWORD` の Gradle properties が揃った時だけ debug signingConfig を上書きします。通常のローカル debug build は従来どおり Android の既定 debug keystore を使います。
